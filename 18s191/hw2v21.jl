...
student = (name = "Παναγιώτης Γεωργακόπουλος", kerberos_id = "jazz")
...
# ╔═╡ 2511bc12-1918-11eb-2336-35437333fd8e
reflect([i*j for i ∈ 1:3, j ∈ 1:3])
...
function least_energy(energies, i::Int64, j::Int64)
	E = energies[i, j]
	I, J = size(energies)
	if i == I
		return E, j
	...
	minJ = l
	for j′ ∈ l:h
		E′, _ = least_energy(energies, i + 1, j′)
		cur = E + E′
		if cur < minE
			minE = cur
			minJ = j′
...
function recursive_seam(energies, starting_pixel::Int64)
	m, n = size(energies)
	...
	for i ∈ 1:m
		push!(seam, p)
		E, p = least_energy(energies, i, p)
	end
	return seam
end

# ╔═╡ 8e146eaa-1934-11eb-070c-25ebe17c85a9
recursive_seam([1 2 2; 4 5 6; 7 8 9], 2)

# ╔═╡ c7b06e56-1935-11eb-1761-87e4338cc9ac
recursive_seam([1 2 2; 4 5 6; 17 8 9], 2)
...
function memoized_least_energy(energies, i::Int64, j::Int64, memory::Dict{Tuple{Int,Int}, Float64})
	m, n = size(energies)
	if haskey(memory, (i,j))
		return get(memory, (i,j), nothing) 
	end
	E = energies[i, j]
	I, J = size(energies)
	if i == I
	...
	minJ = l
	for j′ ∈ l:h
		E′= memoized_least_energy(energies, i + 1, j′, memory)
		cur = E + E′
		if cur < minE
			minE = cur
			minJ = j′
...
function recursive_memoized_seam(energies, starting_pixel::Int64)
	...
	for i ∈ 1:m-1
		...
		minJ = l
		for j ∈ l:r
			E_j = memoized_least_energy(energies, i + 1, j, memory)
			if E_j <= minE
				minE = E_j
				minJ = j
			end
...
# ╔═╡ cbc29d8e-195d-11eb-1353-4920fd23b9b4
recursive_memoized_seam([1 2 3; 4 5 6; 7 7 10], 1)
...
# ╔═╡ ff7c6436-191d-11eb-19d2-15e626339b96
energy(pika)

# ╔═╡ f8d85350-1957-11eb-3466-9bc90234f753
least_energy(energy(pika), 3,3)
...
if shrink_recursive
	recursive_carved = shrink_n(pika, 3, recursive_memoized_seam)
