...
html"<style>.cm-s-erlang-dark.CodeMirror { background: #002240; color: white; }
...
.cm-s-erlang-dark .CodeMirror-matchingbracket { outline:1px solid grey; color:white !important; }</style>"
...
# ╔═╡ df4b59c0-10a4-11eb-21a6-d149053939fb
html"<style>pluto-notebook {--font-variant-ligatures: contextual;}</style>"
...
function vecvec_to_matrix(vecvec)
	N = length(vecvec)
	M = length(vecvec[1])
	mat = Array{typeof(vecvec[1][1]), 2}(undef, N, M)
	for i ∈ 1:N
		for j ∈ 1:M
			mat[i, j] = vecvec[i][j]
		end
...
# ╔═╡ 886c1c40-10a7-11eb-3810-47df29d5168a
typeof([[1,2],[1,2]][1][1])
...
function matrix_to_vecvec(matrix)
	N, M = size(matrix)
	return [matrix[:, i] for i ∈ 1:M]
end

# ╔═╡ 70955aca-ed6e-11ea-2330-89b4d20b1795
matrix_to_vecvec([6 7; 8 9])

# ╔═╡ c18cc8d6-12c2-11eb-3aa5-c13ccd953fad
...
function mean_colors(image)
	return (mean((d -> d.r).(image)), mean((d -> d.g).(image)), mean((d -> d.b).(image)))
end
...
# ╔═╡ 90f2e364-10aa-11eb-229a-117077ca9e0d
...
begin
	function noisify(x::Number, s)
		return myclamp(x + 2s*(rand() - .5), 0, 1)
	end
...
The image is unrecognisable with intensity 4.

We need intensities > 1 because for every intentity i, 1/i of pixels get intensified less than 1 and 1/(2*i) get intensified less than 50%.
...
function convolve_vector(v, k)
	l = length(k) ÷ 2
	return [sum(extend(v, j) * k[j + l - i + 1] for j in i - l:i + l) for i in 1:length(v)]
end
...
function gaussian_kernel(n)
	s = sqrt(1/2/log(2))
	l = n ÷ 2
	g = x -> 1 / sqrt(2π*s^2) * exp(-x^2/(2s^2))
	tsample = [mean(g.(i-.01:1/n^2:i+.01)) for i in -l:l]
	return tsample/sum(tsample)
end
...
@bind gaussian_kernel_size_1D Slider(1: 100, show_value=true) # change this value, or turn me into a slider!

# ╔═╡ f210f382-10b1-11eb-0677-f9ad797cb0fb
begin
	Pkg.add("Plots")
	plot(gaussian_kernel(gaussian_kernel_size_1D))
end
...
# ╔═╡ e9202050-12c2-11eb-040e-f19689250b9d
...
# ╔═╡ e3f9413e-13d9-11eb-0dc1-bd9b9485cd06
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	return [
		sum(
			[extend_mat(M, i+ii, j + jj) * K[ii + 1, jj + 1]
			for ii ∈ 0:(I-1)
			for jj ∈ 0:(J-1)]
		) for i ∈ 1:mI, j ∈ 1:mJ
	]
...
# ╔═╡ 7e2336c4-13db-11eb-0a1f-c57905e50755
function gaussian_kernel_2d(n)
	s = sqrt(1/2/log(2))
	l = n ÷ 2
	g = (x, y) -> 1 / sqrt(2π*s^2) * exp(-(x^2 + y^2)/(2s^2))
	tsample = [mean(g.(i-.01:1/n^2:i+.01, (j-.01:1/n^2:j+.01))) for i in -l:l, j in -l:l]
	return tsample/sum(tsample)
end

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image, k=2)
	
	return convolve_image(image, gaussian_kernel_2d(k)) 
end
...
# ╔═╡ f9bedfbe-13df-11eb-20f9-c1f9fa2cabdd
Gx = [1 0 -1
	  2 0 -2
      1 0 -1]

# ╔═╡ 13ff9404-13e0-11eb-36b8-e95a531030cf
Gy = Gx'

# ╔═╡ 20dacd88-13e0-11eb-24c7-19c8dff906cd
E = ((x, y) -> sqrt(x^2 + y^2))

# ╔═╡ 1c46d338-13e1-11eb-1887-277903457226
begin
	function A(x::RGB)
		return sqrt(x.r^2 + x.g^2 + x.b^2)
	end
	function A(x::Gray)
		return x
	end
...
function with_sobel_edge_detect(image)
	...
	return Gray.(E.(A.(convolve_image(image, Gy)), A.(convolve_image(image, Gx))))
end

# ╔═╡ 1bf94c00-ee19-11ea-0e3c-e12bc68d8e28
with_sobel_edge_detect(with_sobel_edge_detect(with_gaussian_blur(with_sobel_edge_detect(philip),10)))
...
@bind gauss_raw_camera_data camera_input(max_size=120)
