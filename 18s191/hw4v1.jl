...
# ╔═╡ 129b661e-291c-11eb-17fe-9158362bb487
using Statistics
...
student = (name = "Παναγιώτης Γεωργακόπουλος", kerberos_id = "hades")
...
# ╔═╡ 932d27d6-28d9-11eb-010f-db396cd90a9a


# ╔═╡ fdf2c12c-28d9-11eb-3065-1749745c1a56
Base.UUID("12cc2940-0403-11eb-19a7-bb570de58f6f")
...
Trump gets antibodies (perfect cure)
"""
...
# ╔═╡ f1242ee6-27f7-11eb-05e0-15ee6c42f608
md"""
| select stuff   | $(@bind y Slider(1:10))     | $(@bind z Slider(1:10, 5, false))     |
| :------------- | :----------  | :----------- |
...
large_experiment = do_experiment(0.25, 100_000) 
...
@bind p_interactive Slider(0.01:0.05:1, .50, true)
		

# ╔═╡ a655c336-278c-11eb-19e3-5de3ebf63b7a
@bind N_interactive Slider(1000:50:10000, 1000, true)

# ╔═╡ ff45f0b0-278c-11eb-37ac-cb199402d627
...
begin
	res = frequencies_plot_with_mean(do_experiment(p_interactive, N_interactive))
	T_res = 1:62
	plot!(res, T_res, (k->(p_interactive)*(1-p_interactive)^(k-1)).(T_res))
end
...
begin
	ps = 0.01:0.01:1
	ts = sum.(do_experiment.(ps, 10000))/10000
	p = plot(ps, ts)
	one_by_x = (x -> 1/x).(ps)
	plot!(p, ps, one_by_x)
end
...
# ╔═╡ 99fe78cc-27fe-11eb-1028-91f6bd47d0e6
typeof(test_status)
...
# ╔═╡ 80f7fa8c-284d-11eb-1ebd-990a9b673619
rand([1,2], 5)
...
# ╔═╡ dd541764-2851-11eb-2541-e91932b37345
md"""

Infection                        | Recovery                        
:---                              | :---
$(@bind pv Slider(0.001:0.005:1, 0.001, true)) | $(@bind rv Slider(0.001:0.005:1,0.001, true))
"""
...
# ╔═╡ 7dc6a046-2803-11eb-0051-f9f8836f06a2
md"""test[^1]
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	S = length(simulations)
	T = length(first(simulations).S) 
	p = plot()
	s = sum([simulations[s].S[d] for s ∈ 1:S, d ∈ 1:T], dims=1)[1, :]/S
	i = sum([simulations[s].I[d] for s ∈ 1:S, d ∈ 1:T], dims=1)[1, :]/S
	r = sum([simulations[s].R[d] for s ∈ 1:S, d ∈ 1:T], dims=1)[1, :]/S
	plot!(p, 1:T, s, alpha=.5, label=nothing)
	plot!(p, 1:T, i, alpha=.5, label=nothing)
	plot!(p, 1:T, r, alpha=.5, label=nothing)
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	sstd = [std([simulations[s].S[d] for d ∈ 1:T]) for s ∈ 1:S]
	istd = [std([simulations[s].I[d] for d ∈ 1:T]) for s ∈ 1:S]
	rstd = [std([simulations[s].R[d] for d ∈ 1:T]) for s ∈ 1:S]
	
	plot!(p, 1:T, s, yerr=sstd, alpha=.1, label=nothing)
	plot!(p, 1:T, i, yerr=istd, alpha=.1, label=nothing)
	plot!(p, 1:T, r, yerr=rstd, alpha=.1, label=nothing)
...
begin
	function interact!(agent::Agent, source::Agent, infection::InfectionRecovery)
		# your code here
		if bernoulli(infection.p_infection) && source.status == I && agent.status == S
			agent.status = I
			source.num_infected += 1
		elseif bernoulli(infection.p_recovery) && agent.status == I
			agent.status = R
 		end
	end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i ∈ 1:T
		sweep!(A, infection)
		push!(S_, length(filter(x -> x.status == S, A)))
		push!(I_, length(filter(x -> x.status == I, A)))
		push!(R_, length(filter(x -> x.status == R, A)))
	end
	
	return (S=S_, I=I_, R=R_)
end
...
simulation(100, 1000, InfectionRecovery(pv, rv))
...
sim01 = let
	...
	N = 100
	T = 1000
	sim = simulation(N, T, InfectionRecovery(pv, rv))
...
let
	...
	yyyy = let
		D = length(simulations[1].I)
		S = length(simulations)
		sum([simulations[s].I[d] for s ∈ 1:S, d ∈ 1:D], dims=1)[1, :]/S
	end
	plot!(p, 1:1000, yyyy, alpha=1, lw=3, label=nothing)
end
...
let 
	simulations = [simulation(3, 20, Reinfection(0.04, 0.02)) for i ∈ 1:20]
	sir_mean_plot(simulations)
end
